import gmpy2  # for converting from decimal to base 9
import sys
...
def dec2nonal_triple(triple):
    if(triple == "0"):
        return "0"
    return str(gmpy2.digits(int(triple), 9)).zfill(3)
...
def print_output_as_readable(result):
    # Decoded output: copy and pasted from the results
    print "Puzzle solution\n"
    # result is the value of the output of the SAT solver
    ...
    res = [x for x in res if int(x) > 0]

    encoded = readable_format(stringify(res))
...
if __name__ == "__main__":
    ...
    fo.readline() # toss away the first line of the file
    result = fo.read()
    result.strip("\n")
    print_output_as_readable(result)
